...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for l in range(N+N):
          z_l = out[l]
          if k!=l:
            denom_kkN += torch.exp(sim(z_k, z_l) / tau).sum()

          if kN != l:
            denom_kNk += torch.exp(sim(z_k_N, z_l) / tau).sum()
        l_kkN = - torch.log(torch.exp(sim(z_k, z_k_N)/tau) / denom_kkN)
        l_kNk = - torch.log(torch.exp(sim(z_k_N, z_k)/tau) / denom_kNk)
        total_loss += l_kkN + l_kNk
...
def sim_positive_pairs(out_left, out_right):
    ...
    norms = (torch.linalg.norm(out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True))
    pos_pairs = ( pos_pairs / norms ).sum(dim=1, keepdim=True)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    ##############################################################################
    # DONE: Start of your code. Follow the hints.                                #
    ##############################################################################
    N2 = N*2
    ...
    sim_pairs = sim_matrix[range(2*N), [*range(N, N2), *range(0, N)]]
    #sim_pairs = torch.cat( (sim_positive_pairs(out_left, out_right), \
    #  sim_positive_pairs(out_right, out_left)) )
    #print('sim_pairs:', sim_pairs.shape, sim_pairs)
    #print('sim_matrix:', sim_matrix.shape, sim_matrix)
    #print('out_left:', out_left.shape)
    #print('mask:', mask)
    #print('sim_positive_pairs 1:', sim_positive_pairs(out_left, out_right))
    #print('sim_positive_pairs 2:', sim_positive_pairs(out_right, out_left))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
