...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, next_h, Wx, Wh, _ = cache

    # derivative of tanh(x) is 1 - tanh(x)**2
    dtanh = dnext_h * (1 - next_h **2)

    dx = dtanh.dot(Wx.T)       # Wx (D, H)
    dprev_h = dtanh.dot(Wh.T)  # Wh (H, H)
    dWx = x.T.dot(dtanh)       # x (N, D)
    dWh = prev_h.T.dot(dtanh)  # prev_h (N, H)
    db = dtanh.sum(axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    inputs = np.moveaxis(x, 1, 0)  # input is (T, N, D)
    cache=[]
    ...
    for t, x in enumerate(inputs):
      #print("t:", t, "\nx:", x)
      next_h, step_cache = rnn_step_forward(x, prev_h, Wx, Wh, b)
      h[t] = next_h   # next_h (N, H)
      cache.append(step_cache)
      _, _, prev_h, Wx, Wh, b = step_cache  # prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros((H, H))
    db = np.zeros(H,)
    inputs = np.moveaxis(dh, 1, 0)  # input is (T, N, H) 
    for t in range(T-1, -1, -1):
      dxt, dh0, dWxt, dWht, dbt = rnn_step_backward(inputs[t] + dh0, cache[t])
      dx[t] = dxt
      dWx += dWxt
      dWh += dWht
      db += dbt
...
def word_embedding_backward(dout, cache):
    ...
    # basically dX += dout(at indices x)
    np.add.at(dW, x, dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    ifog[:, 0:3*H] = sigmoid(ifog[:, 0:3*H])
    ifog[:, 3*H:4*H] = np.tanh(ifog[:, 3*H:4*H])
    #i = ifog[:, 0:H]
    #f = ifog[:, H:2*H]
    #o = ifog[:, 2*H:3*H]
    #g = ifog[:, 3*H:4*H]
    ifog = np.hsplit(ifog, 4)
    i = ifog[0]
    f = ifog[1]
    o = ifog[2]
    g = ifog[3]
    ...
    #print(prev_c.shape, prev_h.shape)
    next_c = f * prev_c + i * g
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      dxt, dh, dc, dWxt, dWht, dbt = lstm_step_backward(inputs[t] + dh, dc, cache[t])
      dx[t] = dxt
      dWx += dWxt
      dWh += dWht
      db += dbt
