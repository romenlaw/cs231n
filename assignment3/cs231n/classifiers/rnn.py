...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2) word embedding, captions_in (N, T), output (N, T, D)
        ...
        # (4) temporal affine, output (N, T, V)
        ...
        print('N, T, D, H, V, W, M:', features.shape[0], captions.shape[1]-1, \
          features.shape[1], b_proj.shape[0], len(self.word_to_idx), \
          W_embed.shape[1], b_vocab.shape[0] \
          )
        print(f'features: {features.shape}, W_proj: {W_proj.shape}, b: {b_proj.shape}')
        print(f'captions_in: {captions_in.shape}, W_embed: {W_embed.shape}, embedded: {embedded_words.shape}')
        print(f'h0: {h0.shape}, Wx: {Wx.shape}, Wh: {Wh.shape}, b: {b.shape}, h: {h.shape}')
        print(f'W_vocab: {W_vocab.shape}, b_vocab: {b_vocab.shape}, out: {x.shape}')
        print(f'captions_out: {captions_out.shape}, mask: {mask.shape}, loss: {loss}')
        grads = {
          "W_proj": dW_proj,
          "b_proj": db_proj,
          "W_embed": dW_embed,
    ...
    def sample(self, features, max_length=30):
        ...
        x = np.repeat(self._start, N) # initial word (N, )
        ...
        for t in range(max_length):
          ...
          if self.cell_type == 'rnn':
            ...
            h, _,_ = lstm_step_forward(x, h, np.zeros_like(h), Wx, Wh, b)
                
          scores, _ = affine_forward(h, W_vocab, b_vocab)  # (N, V)
          #print(scores)
          #print(scores.shape)
          x = np.argmax(scores, axis=1)
          #print(x, decode_captions(x, data['idx_to_word']))
