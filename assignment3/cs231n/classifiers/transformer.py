...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # DONE: Implement the forward function for CaptionTransformer.             #
        # A few hints:                                                             #
        ...
        # (N, T, W)
        embedded_captions = self.positional_encoding( self.embedding(captions) )
        # (N, D) -> (N, W) -> (N, 1, W)
        projected_features = self.visual_projection( features ).unsqueeze(1)

        # (T, T) mask out the future - i.e. w.r.t. T
        tgt_mask = torch.tril( torch.ones(T, T) )

        # out is (N, T, W)
        out = self.transformer(tgt = embedded_captions,
