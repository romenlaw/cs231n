...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # Z is noise, x is real data
    N = logits_real.shape[0]
    loss_real = bce_loss(logits_real, torch.ones(N).type(dtype))
    loss_fake = bce_loss(logits_fake, torch.zeros(N).type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = 0.5 * (scores_real - 1).square().mean()
    loss_fake = 0.5 * scores_fake.square().mean()
    loss = loss_real+loss_fake
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    return nn.Sequential(
      ...
      Unflatten(), # default para values are -1, 128, 7,7 already
