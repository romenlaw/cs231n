...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            # this takes 51s to run with sqrt; 38s without
            # **2 seems faster than np.square (38s vs 53s)
            dists[i,:] = np.sum((self.X_train - X[i,:])**2, axis = 1)

            # partial vectorisation, still takes 38s
            #Xte = X[i, np.newaxis]
            #dists[i, :] = np.sum((Xte - self.X_train)**2, axis = 1)
