...
def svm_loss(x, y):
    ...
    scores_yi = x[np.arange(num_trains), y][:, np.newaxis]  # dim is (N, 1)
    margins = np.maximum(0, x - scores_yi + 1)
    margins[np.arange(num_trains), y] = 0
    loss = np.mean( np.sum(margins, axis=1) )
    
    # dLi/dxj = 1
    # dLi/dx_yi = -n, where n is number of classes-1
    dx = np.ones_like(x)
    dx[np.arange(num_trains), y] = 0
    dx[np.arange(num_trains), y] = -np.sum(dx, axis=1)
    dx /= num_trains
