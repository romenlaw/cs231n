...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # from loss() function:
        stride = 1
        pad = (filter_size - 1) // 2    # filter_size is basically HH and WW
        filter_steps = 1 + (H + 2 * pad - filter_size) // stride  #  filter_steps is H_prime, W_prime
        # first layer: conv layer
        W1 = np.random.normal(loc=0, scale=weight_scale, size=(num_filters, C, filter_size, filter_size))
        b1 = np.zeros(shape=num_filters)

        # max_pool layer from loss() function:
        pool_height = pool_width = 2
        stride = 2
        pool_steps = 1 + (filter_steps - pool_height) // stride # height and width are same
        # given the above strides, filter_size, padding, the pool_steps = H*W//4

        # 2nd layer: hidden layer 
        input_dim2 = num_filters * pool_steps * pool_steps
        W2 = np.random.normal(loc=0, scale=weight_scale, size=(input_dim2, hidden_dim))
